...
namespace BilibiliLiver.Api
{
    ...
    {
        private readonly ILogger<LiveApi> _logger;

        public LiveApi(ILogger<LiveApi> logger)
        {
            _logger = logger;
        }
        ...
        public async Task<string> GetRoomId(Account user)
        {
            ...
            try
            {
                string cookies = GetCookie(user);
                return "0";
                using (HttpResponseMessage response = await new HttpClient().SendAsync(HttpMethod.Get, _getRoomIdApi, null, PCHeaders, cookies))
                {
